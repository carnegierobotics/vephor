...
int main()
{
    Window window(-1, -1, "physics solids");
    Verlet phys(30.0);
	...
	/*for (size_t i = 0; i < 3; i++)
    {
		auto sphere_obj = make_shared<Mesh>(
			sphere_mesh_data,
			Transform3(
				Vec3(0.0,-15.0+15*i,-70),
				Vec3(0.0,0.0,0.0)
			),
			Vec3(0.0,1.0,1.0));
		window.add(sphere_obj);

		phys.add(sphere_obj, Verlet::makeSphere(1.0f), 1.0f);
	}*/
	...
	auto cube_1 = make_shared<Mesh>(
		cube_mesh_data,
		Vec3(0.25,0.5,0.5));
	auto cube_1_render = window.add(cube_1, TransformSim3(
		Vec3(0.0,0.0,-50.0),
		Vec3(M_PI/3.0,0.0,0.0),
		10.0f
	));
	
	auto cube_2 = make_shared<Mesh>(
		cube_mesh_data,
		Vec3(0.25,0.5,0.5));
	auto cube_2_render = window.add(cube_2, TransformSim3(
		Vec3(0.0,13.0,-40.0),
		Vec3(M_PI/3.0,0.0,0.0),
		5.0f
	));
	
	auto cube_3 = make_shared<Mesh>(
		cube_mesh_data,
		Vec3(0.25,0.5,0.5));
	auto cube_3_render = window.add(cube_3, TransformSim3(
		Vec3(0.0,-13.0,-60.0),
		Vec3(M_PI/3.0,0.0,0.0),
		5.0f
	));
    ...
    phys.add(sphere_1_render, Verlet::makeSphere(1.0f), 0.0f);
	phys.add(sphere_2_render, Verlet::makeSphere(1.0f), 0.0f);
	phys.add(cube_1_render, Verlet::makeSolid(cube_solid), 0.0f);
	phys.add(cube_2_render, Verlet::makeSolid(cube_solid), 0.0f);
	phys.add(cube_3_render, Verlet::makeSolid(cube_solid), 0.0f);
